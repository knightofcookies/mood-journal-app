	...
	<div class="h-full bg-background">
		...
		<div class="mb-8">
			...
			<div class="flex items-center gap-4 text-sm text-muted-foreground">
				<span>{data.pagination.totalEntries} {data.pagination.totalEntries === 1 ? 'entry' : 'entries'}</span>
				<span>•</span>
				<a href="/journal/new" class="notion-btn notion-btn-primary text-sm">
					+ New Entry
				</a>
			</div>
		</div>
		...
		<div class="mb-6 space-y-3">
			<!-- Search Bar -->
			<div class="flex items-center gap-2 rounded-lg border border-border bg-card px-4 py-2 transition-all focus-within:border-foreground/30">
			...
			{#if data.topTags.length > 0}
				<div class="flex flex-wrap items-center gap-2">
					...
					{#each data.topTags.slice(0, 8) as tag}
						<button
							onclick={() => { tagFilter = tag.name; updateFilters(); }}
					...
					{#if tagFilter}
						<button
							onclick={() => { tagFilter = ''; updateFilters(); }}
		...
		{#if data.entries.length === 0}
			<div class="mt-16 text-center">
				...
				<a href="/journal/new" class="notion-btn notion-btn-primary">
					Create First Entry
				</a>
			</div>
		{:else}
			<div class="space-y-2">
				{#each data.entries as entry (entry.id)}
					<a
						...
						class="notion-card block p-4 group"
					>
						<div class="flex items-start gap-3">
							<!-- Mood Emoji -->
							<div class="mt-0.5 text-2xl">
								{getMoodEmoji((entry as any).mood || 'neutral')}
							</div>

							<!-- Content -->
							<div class="min-w-0 flex-1">
								<div class="mb-1 flex items-center gap-3">
									<h3 class="font-medium capitalize">{(entry as any).mood || 'neutral'}</h3>
									...
									{#if entry.sentimentScore !== null && entry.sentimentScore !== undefined}
										<span
											class="text-base"
											title="Sentiment: {entry.sentimentLabel} ({entry.sentimentScore > 0 ? '+' : ''}{entry.sentimentScore})"
										>
								...
								{#if entry.tags && entry.tags.length > 0}
									<div class="mt-2 flex flex-wrap gap-1">
										{#each entry.tags.slice(0, 5) as tag}
											<span class="inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-xs bg-accent text-foreground">
												{#if tag.type === 'entity'}
													<span>👤</span>
												{/if}
												{tag.name}
							...
							<!-- Arrow Icon -->
							<svg class="mt-1 h-4 w-4 flex-shrink-0 text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity" fill="none" stroke="currentColor" viewBox="0 0 24 24">
