#!/usr/bin/env node
/*
Dev-only integration test for lockout behavior.
This script mutates the DB to clear the lock after failed attempts so a successful login can be performed deterministically.
Run: node scripts/test-lockout-dev.js
*/
...
async function main() {
	...
	// For deterministic testing, clear the lock so we can perform a successful login and verify reset
	console.log('Clearing locked_until locally so successful login can proceed (dev-only)');
	db.prepare('UPDATE user SET locked_until = ? WHERE username = ?').run(0, testUser);
	await wait(200);
