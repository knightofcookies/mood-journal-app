...
{#if showPrompt}
  <div class="fixed bottom-4 right-4 max-w-sm bg-white dark:bg-gray-800 shadow-xl rounded-lg p-4">
    <button onclick={() => showPrompt = false} class="absolute top-2 right-2">✕</button>
    <h3 class="font-bold mb-2">Install Mood Journal</h3>
    <p class="text-sm mb-4">Get quick access and offline support!</p>
...
export async function updateUserProgress(userId: string, action: string) {
  ...
  const newlyUnlocked = achievements.filter(a => 
    a.unlocked && !existing.some(e => e.achievementId === a.id)
  );
  ...
  const currentProgress = await db.select().from(userProgress).where(eq(userProgress.userId, userId)).get();
...
<div class="max-w-6xl mx-auto p-6">
  ...
  <p class="text-muted-foreground mb-8">Level {data.progress.level} • {data.progress.currentXP} / {data.progress.xpToNextLevel} XP to next level</p>
        ...
        {#each achievements as achievement}
          <Card.Card class={achievement.unlocked ? '' : 'opacity-50'}>
            ...
            <Card.CardContent>
              ...
              {#if !achievement.unlocked}
                <Progress.Root value={achievement.progress} max={achievement.requirement} class="mt-4">
                  <Progress.Indicator />
                </Progress.Root>
              {/if}
              <p class="text-sm text-muted-foreground mt-2">+{achievement.xp} XP</p>
            </Card.CardContent>
          </Card.Card>
        {/each}
      </div>
...
<div class="max-w-4xl mx-auto p-6">
  <h1 class="text-3xl font-bold mb-2">Wellness Recommendations</h1>
  ...
  {#each data.recommendations as rec}
    <Card.Card class="mb-4">
      ...
      <Card.CardContent>
        <Card.CardDescription class="text-base mb-4">{rec.description}</Card.CardDescription>
...
<div class="voice-recorder">
  ...
  {#if transcript}
    <div class="mt-4 p-4 border rounded">
      <p class="text-sm text-muted-foreground mb-2">Transcript:</p>
      <p>{transcript}</p>
    </div>
  {/if}
</div>
...
<nav class="fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 md:hidden z-50">
  <div class="flex justify-around items-center h-16">
    {#each navItems as item}
      <a
        href={item.href}
        class="flex flex-col items-center justify-center flex-1 h-full {$page.url.pathname === item.href ? 'text-blue-600 dark:text-blue-400' : 'text-gray-600 dark:text-gray-400'}"
      >
